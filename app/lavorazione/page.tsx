...
export default function LavorazionePage() {
  ...
  const lavoriFiltrati = tasks.filter((task) => {
    ...
    const matchSearch =
      ...
      task.categoria.toLowerCase().includes(searchLower);

    // Mostriamo solo i veicoli in officina (In Corso o Verniciatura)
    // Escludiamo "In Consegna" e "Consegnato"
    return (task.status === "In Corso" || task.status === "Verniciatura") && matchSearch;
  });
              ...
              <div className="px-6 py-5 bg-white border-t border-slate-50 flex items-center justify-between">
                ...
                {deliveringId === task.id ? (
                  <div className="flex gap-2 animate-in slide-in-from-right-2">
                    {/* MODIFICA: Qui ora impostiamo 'In Consegna' invece di 'Consegnato' */}
                    <button 
                      onClick={() => { updateTask(task.id, 'In Consegna'); setDeliveringId(null); }} 
                      className="bg-orange-500 text-white px-4 py-2 rounded-lg text-[10px] font-black hover:bg-orange-600 shadow-md transition-all"
                    >
                      SPOSTA IN CONSEGNA
                    </button>
                    <button onClick={() => setDeliveringId(null)} className="bg-slate-100 text-slate-400 px-3 py-2 rounded-lg transition-colors"><X size={14} /></button>
                  </div>
                ) : (
                  <button onClick={() => setDeliveringId(task.id)} className="flex items-center gap-2 px-5 py-2.5 bg-blue-600 text-white rounded-xl text-[10px] font-black hover:bg-blue-700 transition-all shadow-lg shadow-blue-100 group/btn">
